...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if not electronics or not groceries or not apparel:
            self.stdout.write(self.style.ERROR("Categories not found. Run seed_categories first!"))
            return

        #Define Core Test Scenarios (The "Diverse" Data)
        CORE_PRODUCTS = [
            # --- ELECTRONICS ---
            {
                "name": "iPhone 15", 
        ...
        for p_data in CORE_PRODUCTS:
            ...
            if not exists:
                Product(**p_data).save()
                self.stdout.write(self.style.SUCCESS(f"Created: {p_data['name']}"))

        #BULK DATA (Scenario: Testing Pagination)
        # Create 15 generic items in Electronics so we have 2+ pages (assuming 10 per page)
